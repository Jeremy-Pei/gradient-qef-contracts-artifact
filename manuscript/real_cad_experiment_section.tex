% Generated from frozen JSON and integrated in the post-v5 manuscript.
\section{Real-CAD end-to-end audit}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figures/fig_real_cad_experiment_v1_scaled.pdf}
\caption{Real-CAD audit.  (a) The no-healing STEP/B-rep preflight retains all
528 inputs in the denominator and admits 511 derived meshes.  (b) The fixed
regular-grid run completes 118 of 122 selected models; the four coarse-grid
misses reappear at resolution 32 or 48 in a diagnostic-only ladder.  (c)
Oracle-diagnostic QEF coverage varies by source.  Because the source mesh
provides the reference closest point and pseudo-normal, these coverage values
are not deployable certificates.}
\label{fig:real-cad-audit}
\end{figure}
